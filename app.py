...
def optimize_yield():
    try:
        ...
        if filtered_data.empty:
            return jsonify({"error": f"No data available for {crop} in {state} during {season} season"}), 404
...
def health_check():
    ...
    try:
        ...
        return jsonify({"status": "unhealthy", "error": str(e), "timestamp": pd.Timestamp.now().isoformat()}), 500
