...
def analyze_similar_images(uploaded_image_path):
    ...
    if most_similar_image:
        ...
        logging.info(
            f"Most similar image: {most_similar_image}, Predicted Disease: {predicted_class}, Confidence: {confidence}%"
        )
...
def analyze_all_images_in_dataset():
    ...
    for filename in os.listdir(dataset_path):
        if filename.endswith((".png", ".jpg", ".jpeg")):  # Check for image files
            ...
            logging.info(
                f"Pre-trained model result: {predicted_class} with confidence {confidence}%"
            )  # Log the result from pre-trained model
